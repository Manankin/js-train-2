...
function checkDataType(value) {
  ...
  let answer = `Значення ${value} має тип ${valueType}`;
...
function getDayOfWeek(dayNumber) {
  ...
  switch (dayNumber) {
    ...
    case 7:
      day = 'Sunday';
      return day;
    default: return 'There are no same day in the week'
  }
...
function getVariableType(variable) {
  let message = typeof (variable) == 'number' ? "It's a number" : (
    (typeof (variable) == 'string') ? "It's a string" :
      ((typeof (variable) == 'boolean') ? "It's boolean" : "It's other type"));
  return message;
...
console.log("divide(4)(10)", divide(4)(10)); // Виведе: 0.2


// Задача 15: Створити меморізовану версію функції, яка обчислює площу квадрата на основі довжини його сторони.

function memorizedArea(side) {
  // Ініціалізуємо примітивні змінні для зберігання попереднього аргументу та результату
  let prevSide, prevResult, result;
...
function compose(func1, func2) {
  ...
  return (n) => {return double(increment(n))}
  // Повертаємо func2, в яку передаємо func1 з аргументом n
}
let createCompose = compose(5);
